...
- 模型白名单环境变量统一使用：`SORA_REFERENCE_VIDEO_DOUBLE_PRICE_MODELS`
- Seedance 模型名直接配置到该变量中，不再使用单独的 Seedance 白名单变量
- 模型白名单默认值为空，不配置时任何 Seedance 模型都不会触发本定制双倍计费
- 多个模型用英文逗号分隔，例如：

```bash
SORA_REFERENCE_VIDEO_DOUBLE_PRICE_MODELS=doubao-seedance-2-0-260128-2,doubao-seedance-2-0-260128-3
```
...
- `relay/channel/task/sora/constants.go`
  - 继续从 `SORA_REFERENCE_VIDEO_DOUBLE_PRICE_MODELS` 加载参考视频计费白名单，Seedance 模型名也配置在该变量中
